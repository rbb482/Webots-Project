...
#define POSITION_THRESHOLD 0.1    // Reduced from 1000.0 to 0.1 meters
...
#define CONSECUTIVE_READINGS_REQUIRED 1 // Reduce from 5 to 3
#define MIN_CONFIDENCE_THRESHOLD 0.4  // Lower the initial confidence threshold
...
typedef struct {
    ...
    LightPoint lowest_light_point;  // Add this new member
...
void init_robot(Robot *robot) {
    ...
    robot->lowest_light_point.intensity = INFINITY;  // Initialize to infinity
...
void check_light_intensity(Robot *robot) {
    double sensor_intensities[NUM_LIGHT_SENSORS];
    ...
    if (robot->light_confidence > MIN_CONFIDENCE_THRESHOLD) {
        ...
        if (robot->consecutive_light_readings >= CONSECUTIVE_READINGS_REQUIRED) {
            // Update highest point with more lenient criteria
            if (avg_intensity > robot->highest_light_point.intensity * 0.5) {
                robot->highest_light_point.intensity = avg_intensity;
                robot->highest_light_point.x = robot->current_position.x;
                robot->highest_light_point.y = robot->current_position.y;
            }

            // Update lowest light point
            if (robot->lowest_light_point.intensity == INFINITY ||
                avg_intensity < robot->lowest_light_point.intensity) {
                ...
                printf("Updated LOWEST intensity light: %.3f at (%.3f, %.3f)\n",
                       avg_intensity, robot->current_position.x,
                       robot->current_position.y);
            }

            if (!robot->light_source_found) {
                robot->light_source_found = true;
                //printf("Light source detected at (%.3f, %.3f) with intensity %.3f\n",
                       //robot->current_position.x, robot->current_position.y, avg_intensity);
            }
        }
    }
}
...
bool check_reached_lowest_light(Robot *robot) {
    // Calculate a small threshold around the original lowest light intensity
    double intensity_threshold = robot->lowest_light_point.intensity * 1.1;  // 10% tolerance
    
    // Check if current light readings are close to the original lowest light
...
int main() {
    ...
    while (wb_robot_step(TIME_STEP) != -1 && !should_exit) {
        read_sensors(&robot);
        ...
        if (!exploration_complete) {
            ...
            if (check_maze_completion(&robot)) {
                ...
                printf("Starting return to lowest light intensity area...\n");
                continue;
            }
            ...
            if (!reached_target) {
                // Check if lowest light area is reached using sensor readings
                if (check_reached_lowest_light(&robot)) {
                    // Stop the robot
                    wb_motor_set_velocity(robot.left_motor, 0.0);
                    wb_motor_set_velocity(robot.right_motor, 0.0);
                    
                    // Print final details
                    printf("\nFinal position: (%.3f, %.3f)\n",
                           robot.current_position.x,
                           robot.current_position.y);
                    printf("Reached lowest light intensity area (%.3f)!\n",
                           robot.lowest_light_point.intensity);
