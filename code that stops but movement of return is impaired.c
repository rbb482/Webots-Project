...
// Updated calculate_speeds function with hybrid navigation
void calculate_speeds(Robot *robot, double *left_speed, double *right_speed, bool returning) {
    *left_speed = 0.0;
    *right_speed = 0.0;
    
    if (returning) {
        // Calculate target angle and current angle difference
        double target_angle = calculate_angle_to_target(robot, 
            robot->highest_light_point.x, 
            robot->highest_light_point.y);
        
        // Calculate angle difference and normalize to [-π, π]
        double angle_diff = target_angle - robot->orientation;
        ...
        // Base speed decreases as we get closer to target
        double base_speed = fmin(MAX_SPEED, distance + 0.5);
        
        // Proportional control for rotation
        double rotation_speed = 2.0 * angle_diff;
        
        // Check for obstacles using front sensors
        double front_obstacle = 0.0;
        for (int i = 0; i < NUM_SENSORS; i++) {
            if (i == 0 || i == 7) {  // Front sensors
                double normalized_value = robot->sensor_values[i] / RANGE;
                front_obstacle = fmax(front_obstacle, normalized_value);
            }
        }
        
        // If obstacle detected, blend wall-following behavior
        if (front_obstacle > 0.3) {
            // Calculate wall-following speeds
            double wall_left_speed = 0.0;
            double wall_right_speed = 0.0;
            
            // Apply sensor weights for wall-following
            for (int i = 0; i < NUM_SENSORS; i++) {
                double normalized_value = 1.0 - (robot->sensor_values[i] / RANGE);
                wall_left_speed += SPEED_UNIT * sensor_weights[i][0] * normalized_value;
                wall_right_speed += SPEED_UNIT * sensor_weights[i][1] * normalized_value;
            }
            
            // Blend between target-seeking and wall-following based on obstacle proximity
            double blend = front_obstacle * 2.0;  // More wall-following when closer to obstacles
            blend = fmin(1.0, blend);
            
            *left_speed = (1.0 - blend) * (base_speed - rotation_speed) + 
                         blend * wall_left_speed;
            *right_speed = (1.0 - blend) * (base_speed + rotation_speed) + 
                          blend * wall_right_speed;
        } else {
            // Pure target-seeking behavior
            *left_speed = base_speed - rotation_speed;
            *right_speed = base_speed + rotation_speed;
        }
        
        // Slow down when very close to target
        if (distance < POSITION_THRESHOLD * 2) {
            *left_speed *= 0.5;
            *right_speed *= 0.5;
        }
    } else {
        // Exploration mode - Wall following behavior
        
        // Calculate base speeds using sensor weights
        for (int i = 0; i < NUM_SENSORS; i++) {
            double normalized_value = 1.0 - (robot->sensor_values[i] / RANGE);
        ...
        switch (robot->current_direction) {
            ...
            default:  // Forward (case 0)
                ...
                if (front_left > 0.3 || front_right > 0.3) {
                    // Adjust trajectory to avoid frontal collision
        ...
        // Add momentum to avoid sudden direction changes
